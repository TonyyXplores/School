...
public class Sequence extends Element{
	...
	public Element index( int pos ){ //return the current element at given index
		if( pos < 0 || pos >= length()){  //if position is greater than the sequence length or less than 0, return an error and exit
			System.err.print("Position does not exits");
			System.exit(1);
		}
		Sequence current = this;
	...
	public SequenceIterator begin(){ //return the first element by doing a copy 
		SequenceIterator begin = new SequenceIterator();
		begin.iterator = this.copy();
		return begin;
	}

	public SequenceIterator end(){ //return the last element by copy and check if it null or not
		SequenceIterator end = new SequenceIterator();
		end.iterator = this.copy();
		while(end.iterator.next != null) //keep move on until it hit the end
			end.iterator = end.iterator.next;
		return end;
	}
